...
function jumpingOnClouds(c) {
  let length = c.length;
  // Base case
  if (!c.includes(0) || c.length < 2 || c.length > 100) {
    console.error('Input does not match given constraints!');
  }
  // Jump over clouds;
  let route = [];
  ...
  for (let j = 1; j < route.length; j++) {
    if (route[j] - 1 === route[j - 1] && route[j] + 1 === route[j + 1]) {
      route = route.filter((r) => r !== route[j]);
    }
  }
  console.log(route);
